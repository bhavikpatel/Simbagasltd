...
public class CylinderListSortingActivity extends AppCompatActivity {
    Context context;
    RelativeLayout rvBlank;
    SharedPreferences spSorting;
    Button btnCancel,btnApply;
    RadioGroup cyldSortRadioGroup,cyldRadioGroupAscDsc;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cylinder_list_sorting);
        context=this;
        ...
        cyldSortRadioGroup=findViewById(R.id.companyRadioGroup);
        cyldRadioGroupAscDsc=findViewById(R.id.userRadioGroupAscDsc);
        sortbyName=new ArrayList<>();
        sortbyName.add("Cylinder No");
        sortbyName.add("Manufacturing Company");
        sortbyName.add("Valve Company Name");
        for(int i=0;i<sortbyName.size();i++){
            ...
            cyldSortRadioGroup.addView(radioButton,i,rprms);
            if(sortbyName.get(i).equals(spSorting.getString("text1","Cylinder No").trim())){
        ...
        for(int i=0;i<sortbyAscDsc.size();i++){
            ...
            radioButton.setTextSize(getResources().getDimension(R.dimen.radiolbl));
            radioButton.setButtonTintList(getRadioButtonColors());
            cyldRadioGroupAscDsc.addView(radioButton,i,rprms);
            if(sortbyAscDsc.get(i).equals(spSorting.getString("text2","Decinding").trim())){
                radioButton.setChecked(true);
            }
        }
        cyldSortRadioGroup.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            @Override
        ...
        cyldRadioGroupAscDsc.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                Log.d("checkedId==>",checkedId+"");
                SharedPreferences.Editor editor = spSorting.edit();
