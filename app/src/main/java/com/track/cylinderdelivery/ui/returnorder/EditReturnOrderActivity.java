...
public class EditReturnOrderActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Edit Return Order");
        getSupportActionBar().setTitle(Html.fromHtml("<font color='#734CEA'>Edit Return Order</font>"));
        ...
        edtVehicleno.setText(mapdata.get("driverVehicleNo"));
...
/*        if(Remark.length()==0){
    ...
    private void callGetReturnOrderCylinderList() {
        ...
        String url = MySingalton.getInstance().URL+"/Api/MobReturnOrder/GetReturnOrderCylinderList?search=&pageno=0&totalinpage="+Integer.MAX_VALUE+
                "&SortBy=&Sort=desc&ROid="+ROId;
        Log.d("request==>",url);
        StringRequest stringRequest = new StringRequest(Request.Method.GET,
                url,new Response.Listener<String>() {
            @Override
            public void onResponse(String Response) {
                ...
                try {
                    ...
                    for(int i=0;i<datalist.length();i++){
                        ...
                        map.put("roDetailId",dataobj.getString("roDetailId")+"");
                        map.put("roId",dataobj.getString("roId"));
                        map.put("cylinderIds",dataobj.getString("cylinderIds"));
                ...
                try {
                    ...
                    if(j.getBoolean("status")){
                        ...
                        for(int i=0;i<datalist.length();i++){
                            ...
                            if(dataobj.get("userId").toString().equals(mapdata.get("userId").toString())){
                                delnotepos=i+1;
                                try{
