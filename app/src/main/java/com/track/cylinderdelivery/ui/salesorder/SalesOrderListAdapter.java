...
/*                    if(podetailList.get(pos).get("status").equals("Draft")){
    ...
    public void onBindViewHolder(@NonNull SalesOrderListAdapter.ViewHolder holder, int position) {
        ...
        holder.txtUserName.setText("User Name: "+podetailList.get(position).get("username")+
                "\nQuantity: "+podetailList.get(position).get("quantity")+
                "\nCreated by: "+podetailList.get(position).get("soGeneratedBy")+
                "\nCreated on: "+podetailList.get(position).get("strDNDate"));
